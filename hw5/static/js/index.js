...
let init = (app) => {
    ...
    app.set_thumbs_up = function(post_idx){
        let post = app.vue.posts[post_idx]
        post.thumbs_up = 1;
        axios.post(set_thumbs_up_url, {post_id: post.id, rating: 1} );
    };
    ...
    app.methods = {
        ...
        set_thumbs_up: app.set_thumbs_up,
    };
        ...
        axios.get(load_posts_url).then(function (response) {
            ...
            for (let p of app.vue.posts) {
                axios.get(get_rating_url, {params: {"post_id": p.id}})
                    .then((result) => {
                        p.thumbs_up = result.data.rating;
                        p.thumbs_down = result.data.rating;
                    });
            }
